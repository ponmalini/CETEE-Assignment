...
function Designation() {
    ...
    const fetchDesignation = () => {
        axios
            ...
            .then((response) => {
                setList(response.data.designation || []);
            })
            .catch((err) => {
                console.error('Error fetching data:', err);
                Swal.fire({
    ...
    const handleNew = (title, id, name, description,department) => {
        setNewTitle(title);
        setName(name);
    ...
    const addData = async () => {
        ...
        try {
            ...
            const data = { name: Name, description: Description,department:Department };

            const result = await axios.post('http://localhost:5000/designation/add', data);
    ...
    const updateData = async () => {
        ...
        try {
            ...
            const data = { name: Name, description: Description,department:Department, _id: id };

            const result = await axios.put('http://localhost:5000/designation/update', data);
            console.log('Response:', result.data);
            Swal.fire({
                icon: 'success',
                title: 'Saved Successfully',
    ...
    const deleteData = (id) => {
        ...
        if (result) {
            axios
                .delete(`http://localhost:5000/designation/delete/${id}`)
                .then(() => {
                    Swal.fire({
                        icon: 'success',
                        ...
                        <tr>
                            ...
                            <th scope="col" style={{ width: '12rem' }}>
                                <button type="button" className="btn btn-success" data-bs-toggle="modal" data-bs-target="#exampleModal"
                                    onClick={() => handleNew('New Designation', 0, '', '',departmentList[0].name)}>
                                    Add
                                </button>
                            </th>
                        </tr>
                            ...
                            <tr key={item._id}>
                                ...
                                <td>
                                    <button type="button" className="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal"
                                        onClick={() => handleNew('Edit Designation', item._id, item.name, item.description,item.department)}>
                                        Edit
                                    </button>

                                    <button type="button" className="btn btn-danger m-1" onClick={() => deleteData(item._id)}>
                                        Delete
                                    </button>
                                </td>
                            </tr>
                            ...
                            <div className="modal-body">
                                ...
                                <div class="mb-3 row">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        <button type="button" class="btn btn-success" onClick={() => newTitle == 'New Designation' ? addData() : updateData()}>Save</button>
                                    </div>
                                </div>
                            </div>
                        </div>
